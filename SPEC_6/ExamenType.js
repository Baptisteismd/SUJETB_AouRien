...
// Fonction permettant à l'utilisateur de choisir le fichier qu'il souhaite étudier
function demanderNomFichier() {
    // L'utilisateur sélectionne le nom du fichier qu'il recherche
...
// Fonction vérifiant s'il est cohérent rechercher le fichier indiqué par l'utilisateur
async function analyserFichier() {
    try {
        ...
        // Paramètres pour uniquement lire les fichiers à l'extension .gift et dans un répertoire précis
...
async function comparerFichiersDExtension(dossier, extension, NomFichier) {
    // Compteur du nombre total de fichiers (sert pour calculer le nombre moyen de chaque type de question)
    let nbFichiers = 0;
    // Compteur du nombre total de lignes des fichiers
    let totalLignes = 0;
    ...
    try {
        ...
        for (const fichier of fichiersFiltres) {
            ...
            let CompteurQCM = 0;
            //GOOD Compteur le nombre de questions de type Fill in the Blank pour 1 fichier
            let CompteurFillInBlank = 0;
            //GOOD Compteur le nombre de questions de type Matching pour 1 fichier
            let CompteurMatchQ = 0;   
            //GOOD Compteur le nombre de questions de type Essay pour 1 fichier   
            let CompteurEssay = 0;
            //GOOD Compteur le nombre de questions de type à pourcentage pour 1 fichier
            let CompteurRepPourcent = 0;
            //GOOD Compteur le nombre de questions à réponses partielles pour 1 fichier
